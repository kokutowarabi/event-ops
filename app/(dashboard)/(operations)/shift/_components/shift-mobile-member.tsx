        ...
        <div className="relative" style={{ height: MOBILE_TIMELINE_TRACK_HEIGHT }}>
          ...
          {timeOptions
            ...
            .map((slot) => (
              <div
                ...
                data-mobile-time={slot.value}
                className="pointer-events-none absolute left-0 right-0 border-t border-dashed border-border/70"
                style={{ top: getMobileTimelineOffset(slot.minutes) }}
              >
                <span className="absolute left-0 top-0 inline-block w-12 -translate-y-1/2 bg-card pr-2 text-xs leading-none text-muted-foreground">
                  {slot.label}
                </span>
              </div>
            ))}
          ...
          >
            {allMemberShifts.map((shift) => {
              const blockedTop = getMobileSlotOffset(shift.start)
              const blockedHeight =
          ...
          {memberShifts.map((shift) => {
            const top = getMobileTimelineOffset(shift.start)
